...
def create_engine(
    ...
    # Post-processor (rule-based by default, LLM optional)
    from .postprocess import PostProcessPipeline
    postprocessor = PostProcessPipeline(
        enable_llm=getattr(config, "llm_enable", False),
        llm_model_path=getattr(config, "llm_model_path", ""),
    )
